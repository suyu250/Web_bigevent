            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    layer.msg('删除文章成功')
                        ...
                        // 如果没有剩余的数据了， 则让页码 -1 之后，
                        // 再重新调用 initTable() 方法


                    // (就是说通过删除的按钮来判断页面还有几条数据，
                    // 当要删最后一条数据时，先判断页码值， 如果删完之后只剩一页了
                    // 那就然页码值等于1，再去重新获取列表数据， 如果不是1， 就先让页码值
                    // -1之后， 再去获取列表数据， 不然的话获取的还是删除之前的数据，但是删完之后已经
                    // 没有数据了， 就是空的)
                    if (len === 1) {
                        // 如果 len 的值等于1 ， 证明删除完毕之后， 页面上
                        // 就没有任何数据了
                        // 页码值最小必须是1
